...
description: "Manage Apple Reminders from the terminal via the remindctl CLI (add, list, complete) when the user wants iCloud-synced to-dos on macOS."
version: 1.0.0
license: MIT
platforms: [macos]
...
- Scheduling an agent task to run on a timer → use mindpalace-cron (`orchestrator/mpcron.py`), not a Reminders entry
- Calendar events → use Apple Calendar or Google Calendar
- Project task management → use GitHub Issues, Notion, etc.
- If user says "remind me" but means an agent-run scheduled job → clarify first
...
1. When user says "remind me", clarify: Apple Reminders (syncs to phone) vs an agent-run scheduled job via mindpalace-cron
