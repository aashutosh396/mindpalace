---
name: xurl
description: "Operate X/Twitter from the terminal via the official xurl CLI: post, reply, search, DM, follow, upload media, and call any X API v2 endpoint. Use for any X/Twitter action."
version: 1.1.1
...
- The user must fill `~/.xurl` with secrets manually on their own machine. Ensure it is the same `~` (HOME) that the agent's bash subprocesses use.
...
> **HOME mismatch pitfall:** `xurl` reads/writes tokens at `~/.xurl`, resolved from `$HOME`. The agent and the user must run `xurl` with the *same* `$HOME`, or the agent won't see the credentials the user registered. If `xurl auth status` shows no apps/tokens when run by the agent, check that `echo $HOME` matches the HOME the user used during setup; if not, set `HOME=/the/right/home` on the agent's `xurl` calls or move `~/.xurl` accordingly.
...
- **Token storage:** `~/.xurl` is YAML, resolved from `$HOME`. Make sure the agent's `xurl` calls use the same `$HOME` as the user's setup so they find the tokens. Never read or send this file to LLM context.
...
- mindpalace adaptation: reformatted for mindpalace skill conventions (plain bash); safety guardrails preserved verbatim.
