#!/usr/bin/env bash
# GitHub environment detection helper for the github-* skills.
#
# Usage (run from a bash shell, sourced so the vars persist):
#   source <path-to>/github-auth/scripts/gh-env.sh
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif [ -f "$HOME/.mindpalace/.env" ] && grep -q "^GITHUB_TOKEN=" "$HOME/.mindpalace/.env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$HOME/.mindpalace/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
