...
description: "Run iterative, stateful Python through a live Jupyter kernel (hamelnb script) when you need variables to persist across steps for data-science or exploratory work."
version: 1.0.0
license: MIT
...
across executions. Use this instead of one-shot `python -c` / `python script.py`
runs when you need to build up state incrementally, explore APIs, inspect
DataFrames, or iterate on complex code.

## When to Use This vs Plain Bash

| Approach | Use When |
|------|----------|
| **This skill** | Iterative exploration, state across steps, data science, ML, "let me try this and check" |
| `python script.py` / `python -c` (via bash) | One-shot scripts. State does not persist between runs. |
| Plain shell commands (via bash) | Builds, installs, git, process management |
