...
If `gsd-sketch` shows up as a sibling skill (installed via the GSD CLI), prefer **`gsd-sketch`** for the full workflow: persistent `.planning/sketches/` with MANIFEST, frontier mode analysis, consistency audits across past sketches, and integration with the rest of GSD. This skill is the lightweight standalone version — one-off sketching without the state machinery.
...
**Verify variants visually.** Don't just write HTML and hope it renders; open each variant in a browser and look at it. On macOS `open sketches/001-calm-editorial/index.html`, on Linux `xdg-open ...`. If a headless screenshot tool is available (e.g. a Puppeteer/Playwright snippet), capture each variant and inspect the image.

Looking at the rendered page catches layout bugs that pure source inspection misses (e.g. a font import that silently failed, a flex container that collapsed). Fix and re-open until each variant looks right.
...
**Typical sequence for one variant:**
...
open sketches/001-calm-editorial/index.html      # macOS (xdg-open on Linux)
# look at the rendered page; fix any obvious layout issues
```
