...
## Working With Location Pins / Coordinates

When the user supplies a latitude/longitude pair (e.g. from a chat location
pin), pass them straight to `nearby`:

```bash
# Coordinates 36.17, -115.14, "find cafes nearby"
python3 $MAPS nearby 36.17 -115.14 cafe --radius 1500
```

Present results as a numbered list with names, distances, and the
`maps_url` field so the user gets a tap-to-open link. For "open now?"
questions, check the `hours` field; if missing or unclear, verify with a
web search (curl a search engine, or the `claude` CLI with web access)
since OSM hours are community-maintained and not always current.
...
1. Take the lat/lon the user gave
