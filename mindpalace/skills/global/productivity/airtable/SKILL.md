...
Work with Airtable's REST API directly via `curl` in the shell. No MCP server, no OAuth flow, no Python SDK — just `curl` and a personal access token.
...
4. Store the token as the `AIRTABLE_API_KEY` env var (export it in your shell profile, or keep it in your secrets store and source it before use):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the output stays clean. Pipe through `python3 -m json.tool` (usually present) or `jq` (if installed) for readable JSON.
...
- **Always use `curl` in the shell.** Don't try to fetch these endpoints with a generic web-fetch tool (it can't send auth headers) or a browser (needs UI auth and is slow).
- **Make sure `AIRTABLE_API_KEY` is exported** in the shell before each `curl` call (source your secrets/profile if it isn't already in the environment).
