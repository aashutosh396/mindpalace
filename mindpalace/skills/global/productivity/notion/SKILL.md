...
3. Store it as the `NOTION_API_KEY` env var (export it in your shell profile,
   or keep it in your project/secrets store and source it before use):
   ```
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) — wire it up if your client supports MCP and you want streaming Notion access from inside a session, but the curl/`ntn` paths above are enough for most one-shot tasks.
