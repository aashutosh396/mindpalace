#!/usr/bin/env python3
"""Google Workspace API CLI for mindpalace.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing mindpalace-facing JSON contract and falls back to the Python client
...
from _mindpalace_home import get_mindpalace_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MINDPALACE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for mindpalace")
