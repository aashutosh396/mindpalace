...
- web search ("python websocket streaming libraries 2025") — find candidates
- `curl -s <url>` (or a fetch tool) — read the actual docs
- `pip show websockets | grep Version` — check what's installed in the project's venv

For libraries without docs pages, clone and read their `README.md` / `examples/`. Context7 MCP (if the user has it configured) is also a good source.
...
python3 spikes/001-websocket-streaming/main.py
# Observe output, iterate.
```

**Parallel comparison spikes (002a / 002b) — delegate.** When two approaches can run in parallel and both need real engineering (not 10-line prototypes), fan out by launching a fresh `claude -p` subagent per approach (run them in the background):

```bash
claude -p "Build spikes/002a-pdf-parse-pdfjs: ..." &
claude -p "Build spikes/002b-pdf-parse-camelot: ..." &
wait
```
