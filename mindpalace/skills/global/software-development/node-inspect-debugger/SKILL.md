...
- A React/Ink app crashes or behaves wrong and you want to inspect component state pre-render
- Child processes (workers, PTY bridges) misbehave
...
Note: if `chrome-remote-interface` is not a project dependency, install it to a throwaway location so you don't dirty the project:
...
## Debugging an Ink + tsx TUI app

For a terminal UI built with Ink + tsx, two common scenarios:
...
### Debugging a TUI whose Node process is spawned by another runtime

When a Node TUI is spawned from another process (e.g. a Python CLI):
...
### Non-Node child processes

If a child process is Python (worker, PTY bridge) rather than Node, use the `python-debugpy` skill for it. This skill only covers the Node portions (Ink UI, Node clients, tsx-run tests).

## Running Vitest Tests Under the Debugger

```bash
# from the project root — run a single test file paused on entry
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. For interactive stepping, run it in a background/detached process you can write to incrementally; a plain one-shot foreground command works only for non-interactive use, not interactive stepping. Alternatively script it with the CDP driver above.
