...
| **`debugpy`** | Remote / headless / "attach to already-running process." Talks DAP, scriptable from terminal, works for long-lived processes (servers, daemons, worker subprocesses). |
...
- A long-running process (web server, daemon, worker pool) misbehaves and you can't restart it
...
pytest supports dropping into pdb directly:
...
Note: if the project's pytest config enables `pytest-xdist` (`-n N`), pdb does NOT
work under xdist — the prompt never appears and the test just hangs. Add
`-p no:xdist` or run a single test with `-n 0`:

```bash
python -m pytest tests/foo_test.py::test_bar --pdb -p no:xdist
# or, if there's a project test wrapper script, pass the same flags through it.
```

If the project uses a test wrapper that strips env/credentials, debug with raw
`pytest` first to reproduce, then re-confirm under the wrapper before pushing.
...
For long-lived processes: a web server, a daemon, a worker pool, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source .venv/bin/activate   # the target process's virtualenv
pip install debugpy
```
...
The easiest terminal-side DAP client is the VS Code CLI or a small script. From a terminal-only agent you have a few practical options:
...
## Debugging Common Process Types

### Tests
See Recipe 3. If xdist is enabled, always add `-p no:xdist` or run single tests without xdist.

### One-shot CLI / script
Easiest: add `breakpoint()` near the suspect line, then run the program normally. Control returns to your terminal at the pause point.

### A subprocess spawned by a parent (e.g. a backend spawned by a UI)
Options:

**A. Source-edit the child's entry point:**
```python
# near the top of the child's serve()/main()
import debugpy
debugpy.listen(("127.0.0.1", 5678))
debugpy.wait_for_client()
```
Start the parent. It will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.

**B. Use `remote-pdb` at a specific handler:**
```python
from remote_pdb import set_trace
set_trace(host="127.0.0.1", port=4444)   # in the RPC/request handler you want to trap
```
Trigger the matching request, then `nc 127.0.0.1 4444` in another terminal.

### A persistent worker process
Same pattern — `remote-pdb` with `set_trace()` inside the worker's execution path. A persistent worker blocks the first request until you connect; subsequent requests pass through normally unless you re-arm.

### A long-lived server / daemon
Use `remote-pdb` at a request handler, or `debugpy` with `--wait-for-client` if you're restarting the server anyway.
...
8. **Sandboxed test wrappers.** If the project's test wrapper strips credentials or sets `HOME=<tmpdir>`, a bug that depends on user config or real API keys won't reproduce under it. Debug with raw `pytest` first to repro, then re-confirm under the wrapper.
