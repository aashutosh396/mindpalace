...
async def _handle_command(msg, text) -> bool:
    ...
    if cmd == "help":
        _help = (
            "**Commands** — admins only, in the home channel.\n"
            "\n"
            "__Core__\n"
            "`!help` — show this list\n"
            ...
            "\n"
            "__Fleet — rooms & teams__\n"
            "`!architect <goal>` — split a goal into parallel agents + merge (any channel, no setup)\n"
            "`#room <task>` — send a task into an activated room from here\n"
            "`!activate #channel <what it does>` — make a channel its own assistant\n"
            "    ↳ add `readonly`/`full`/`custom` (powers) + `opus`/`sonnet`/`haiku` (model)\n"
            "`!activate #channel orchestrator <focus>` — a room that splits goals across rooms + merges\n"
            "`!scopes` — list activated rooms  ·  `!deactivate #channel` — turn one off\n"
            "\n"
            "__Admin__\n"
            "`!bots` — list bots  ·  `!admins` — who can talk to me\n"
            "`!add-admin @user`  /  `!remove-admin @user` — grant / revoke access\n"
            "`!add-webhook <name> <url>` — add a notify webhook\n"
            "\n"
            "__Settings__\n"
            "`!model opus` — switch model (sonnet/opus/haiku)  ·  `!model` shows current\n"
